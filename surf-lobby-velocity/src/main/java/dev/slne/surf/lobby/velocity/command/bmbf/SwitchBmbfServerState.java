package dev.slne.surf.lobby.velocity.command.bmbf;
...
public class SwitchBmbfServerState extends CommandAPICommand {

  public SwitchBmbfServerState(String commandName) {
    super(commandName);

    withPermission(Permissions.COMMAND_SWITCH_BMBF_SERVER_STATE.getPermission());

    withArguments(new BooleanArgument("enable")
        .replaceSuggestions(ArgumentSuggestions.strings(info ->
            new String[]{VelocityPersistentData.get().isBmbfServerEnabled() ? "false" : "true"})));
  ...
  private void switchState(CommandSource sender, boolean enable) {
    VelocityPersistentData.get().setBmbfServerEnabled(enable);
